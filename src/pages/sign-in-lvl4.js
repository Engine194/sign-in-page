import { useSignUpLvl4 } from "@/hooks/SignUpLvl4/useSignUpLvl4";
import Error from "@/components/Error";
import SocialLogin from "@/components/SocialLogin";

const SignInLvl4 = function (props) {
  const talonProps = useSignUpLvl4();
  const { formState, storeConfig, handlers } = talonProps;
  const {
    onBlurEmail,
    onChangeEmail,
    onBlurPassword,
      ...
      <div className="mx-auto max-w-[930px] bg-[#fafafa] text-center">
        <form className="py-[50px] max-w-[380px] mx-auto" method="POST">
          ...
          <div className="mt-[30px] flex flex-col gap-y-[18px]">
            ...
            <SocialLogin storeConfig={storeConfig?.social_login}/>
          </div>
        </form>
      </div>
    </div>
